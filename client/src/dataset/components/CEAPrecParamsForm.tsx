...
import { DatasetInfoCEAPREC, DatasetParamsCEAPREC, DatasetTypes } from "../../messages";
import { getInitial, getInitialName, parseNumList, withValidation } from "../helpers";
import { OpenFormProps } from "../types";
import Reshape from "./Reshape";

// some fields have different types in the form vs. in messages
type DatasetParamsCEAPRECForForm = Omit<DatasetParamsCEAPREC,
    "type"
    | "path"
    | "nav_shape"
    | "sig_shape"> & {
...
type MergedProps = FormikProps<DatasetParamsCEAPRECForForm> & OpenFormProps<DatasetParamsCEAPREC, DatasetInfoCEAPREC>;

const CEAPrecParamsForm: React.FC<MergedProps> = ({
    values,
    isSubmitting,
    handleSubmit,
    handleReset,
...
export default withValidation<DatasetParamsCEAPREC, DatasetParamsCEAPRECForForm, DatasetInfoCEAPREC>({
    mapPropsToValues: ({ path, initial }) => ({
        name: getInitialName("name", path, initial),
        enable_direct: getInitial("enable_direct", false, initial),
        dtype: getInitial("dtype", "int32", initial),
